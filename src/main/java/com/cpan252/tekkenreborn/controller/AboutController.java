 ...
 * 2. It includes LiveReload server, so we can reload the html pages when
 * changed
 * 3. It also includes H2 console for debugging
 * 4. It disables caching of the templates so we don't need to empty cache on
 * reload
 * 5. Devtools are autodisabled in production so we don't need to disable them
 * manually
 */
